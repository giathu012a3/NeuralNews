    ...
    <div class="flex min-h-screen w-full flex-col">
        ...
        <main class="flex-1 w-full max-w-[1440px] mx-auto p-4 lg:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div class="lg:col-span-8 flex flex-col gap-10">
                ...
                <section class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <article onclick="window.location.href='user/article?id=<%= featured.getId() %>'"
                        ...
                        <div class="absolute bottom-0 left-0 p-6 w-full">
                            ...
                            <p class="text-slate-300 text-sm line-clamp-2 mb-4 hidden md:block"><%= featured.getSummary() %></p>
                            <div class="flex items-center text-xs text-slate-400 gap-3">
                                <span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">schedule</span> <%= featured.getCreatedAt() %></span>
                                <span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">thumb_up</span> <%= featured.getLikesCount() %> thích</span>
                            </div>
                        </div>
                    </article>
                    <% } %>
                        ...
                        <article onclick="window.location.href='user/article?id=<%= art.getId() %>'"
                            ...
                            <div class="w-1/3 h-full rounded-md overflow-hidden bg-slate-100">
                                <div class="w-full h-full bg-cover bg-center transition-transform duration-500 group-hover:scale-110"
                                    style="background-image: url('<%= art.getDisplayImageUrl(request.getContextPath()) %>');"></div>
                            </div>
                ...
                <% List<Article> recs = (List<Article>) request.getAttribute("recommendedArts");
                   if (session.getAttribute("currentUser") != null && recs != null && !recs.isEmpty()) { %>
                <div class="h-px w-full bg-slate-200 dark:bg-slate-700"></div>
                <section>
                    <div class="flex items-center justify-between mb-6">
                        <h3 class="text-xl font-bold flex items-center gap-2 text-slate-900 dark:text-white">
                            <span class="w-1.5 h-6 bg-secondary rounded-full"></span>
                            Dành riêng cho Thịnh
                        </h3>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <% for(Article art : recs) { %>
                        <article onclick="window.location.href='user/article?id=<%= art.getId() %>'" class="cursor-pointer group">
                            <div class="aspect-video rounded-lg overflow-hidden mb-3">
                                <img src="<%= art.getDisplayImageUrl(request.getContextPath()) %>" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                            </div>
                            <h4 class="text-sm font-bold line-clamp-2 group-hover:text-primary"><%= art.getTitle() %></h4>
                        </article>
                        <% } %>
                    </div>
                </section>
                ...
                <section>
                    ...
                    <div class="flex flex-col gap-6">
                        ...
                        <article onclick="window.location.href='user/article?id=<%= art.getId() %>'"
                            ...
                            <div class="sm:w-56 h-40 shrink-0 rounded-lg overflow-hidden relative">
                                <div class="absolute top-2 left-2 bg-black/60 backdrop-blur-sm text-white text-[10px] font-bold px-2 py-0.5 rounded"><%= art.getCategoryName() %></div>
                            ...
                            <div class="flex flex-col flex-1 justify-between py-1">
                                <div>
                                    <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2 group-hover:text-primary transition-colors leading-snug"><%= art.getTitle() %></h3>
                                    <p class="text-slate-600 dark:text-slate-400 text-sm line-clamp-2"><%= art.getSummary() %></p>
                                </div>
                                <div class="flex items-center justify-between text-xs text-slate-400 border-t border-slate-100 dark:border-slate-800 pt-3 mt-3">
                                    <div class="flex items-center gap-4">
                                        <span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">calendar_today</span> <%= art.getCreatedAt() %></span>
                                        <span class="flex items-center gap-1"><span class="material-symbols-outlined text-sm">visibility</span> <%= art.getViews() %> xem</span>
                                    </div>
                    ...
                    <%-- NÚT TẢI THÊM BÀI VIẾT (KHÔI PHỤC) --%>
...
<span class="font-bold">64,230 <span class="text-red-300 text-xs">▼ -1.2%</span></span>
                ...
                <div class="bg-white dark:bg-surface-dark border border-slate-200 dark:border-border-dark rounded-xl p-5">
                    ...
                    <div class="flex flex-col gap-6">
                        ...
                        <a class="group flex gap-3 items-start" href="user/article?id=<%= art.getId() %>">
                            <span class="text-2xl font-black text-slate-200 dark:text-slate-700 leading-none group-hover:text-primary transition-colors"><%= i + 1 %></span>
                            <div>
                                <h4 class="text-sm font-semibold text-slate-800 dark:text-slate-200 leading-snug group-hover:text-primary transition-colors"><%= art.getTitle() %></h4>
                                <span class="text-xs text-slate-500 mt-1 block"><%= art.getViews() %> lượt xem</span>
                ...
                <%-- QUẢNG CÁO GIẢI PHÁP ĐÁM MÂY (KHÔI PHỤC) --%>

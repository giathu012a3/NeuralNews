    ...
    <%@ taglib prefix="c" uri="jakarta.tags.core" %>
        <%@ taglib prefix="fmt" uri="jakarta.tags.fmt" %>
            <%@ taglib prefix="fn" uri="jakarta.tags.functions" %>
                ...
                <c:set var="endItem"
                    value="${(currentPage * limit) > totalArticles ? totalArticles : (currentPage * limit)}" />

                <div id="article-ajax-bundle">
                    <%-- Toolbar --%>
                        <div
                            class="p-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between bg-slate-50/50 dark:bg-slate-900/20 flex-wrap gap-3">
                            ...
                            <p class="text-sm text-slate-500">
                                Hiển thị <span
                                    class="font-semibold text-slate-800 dark:text-white">${startItem}–${endItem}</span>
                                ...
                                <thead>
                                    <tr
                                        ...
                                        <th
                                            ...
                                            Tác giả</th>
                                        <th
                                            class="px-4 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider text-center">
                                            Trạng thái</th>
                                        <th
                                            class="px-4 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider text-center">
                                            <button onclick="AdminContent.toggleSort('views')"
                                                class="flex items-center gap-1 mx-auto hover:text-primary transition-colors">
                                        ...
                                        <th
                                            ...
                                            Người duyệt</th>
                                        <th
                                            class="px-4 py-3 text-[11px] font-bold text-slate-400 uppercase tracking-wider text-right">
                                            Thao tác</th>
                                    </tr>
                                </thead>
                                        ...
                                        <c:when test="${empty articles}">
                                            <tr>
                                                <td colspan="8" class="text-center text-slate-400 py-16">
                                                    <span
                                                        class="material-icons text-5xl block mb-3 text-slate-300">search_off</span>
                                        ...
                                        <c:otherwise>
                                            <c:forEach items="${articles}" var="a">
                                                <c:set var="sumEscaped" value="${fn:replace(a.summary, '\"', '
                                                    &quot;')}" />
                                                <tr
                                                    class="hover:bg-slate-50/80 dark:hover:bg-slate-700/20 transition-colors group ${a.status == 'PENDING' ? 'bg-amber-50/30 dark:bg-amber-900/10' : ''}">
                                                    <td class="px-4 py-4">
                                                        <input
                                                            class="rounded border-slate-300 text-primary focus:ring-primary article-checkbox"
                                                            type="checkbox" value="${a.id}" data-status="${a.status}" />
                                                    </td>
                                                    <td class="px-4 py-4">
                                                        <div class="flex items-center gap-3">
                                                            <img alt="${a.title}"
                                                                class="w-12 h-12 rounded-lg object-cover flex-shrink-0 cursor-pointer"
                                                    ...
                                                    <td class="px-4 py-4 text-sm text-slate-500 dark:text-slate-400">
                                                        ${not empty a.categoryName ? a.categoryName : 'N/A'}</td>
                                                    <td
                                                        class="px-4 py-4 text-sm font-medium text-slate-700 dark:text-slate-300">
                                                        ${not empty a.authorName ? a.authorName : 'Khuyết danh'}</td>
                                                    <td class="px-4 py-4 text-center">
                                                        <span
                                                            class="px-2.5 py-1 text-[10px] font-bold rounded-full whitespace-nowrap ${a.statusBadgeClass}">${a.statusLabel}</span>
                                                    </td>
                                                    <td class="px-4 py-4 text-center text-slate-400">
                                                        <div class="flex items-center justify-center gap-1 text-[12px]">
                                                            <span
                                                                class="material-icons text-[14px]">visibility</span>${a.formattedViews}
                                                        </div>
                                                    </td>
                                                    <td class="px-4 py-4 text-sm text-slate-500">${not empty
                                                        a.reviewerName ? a.reviewerName : (a.status == 'PUBLISHED' ?
                                                        'AI' : '—')}</td>
                                                                ...
                                                                <%-- Status actions --%>
                                                                    <c:choose>
                                                                        <c:when test="${a.status == 'PENDING'}">
                                                                            <button type="button"
                                                                                onclick="handleModerationAction(event, ${a.id}, 'approve')"
                                                                            ...
                                                                            <button type="button"
                                                                                onclick="AdminContent.openRejectModal(${a.id})"
                                                                        ...
                                                                        <c:when test="${a.status == 'PUBLISHED'}">
                                                                            <button type="button"
                                                                                onclick="handleModerationAction(event, ${a.id}, 'archive')"
                                                                    ...
                                                                    <%-- Toggle visibility --%>
                                                                        <c:choose>
                                                                            ...
                                                                            <c:otherwise>
                                                                                <button type="button"
                                                                                    onclick="handleModerationAction(event, ${a.id}, 'hide')"
                                                                                    class="p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-all"
                        ...
                        <%-- Pagination --%>
                            <div
                                ...
                                <p class="text-xs font-medium text-slate-500">Tổng <span
                                        class="text-slate-900 dark:text-white font-bold">${totalArticles}</span> bài
                                    viết</p>
                                    ...
                                    <c:forEach begin="1" end="${totalPages}" var="i">
                                        <c:choose>
                                            <c:when test="${i == currentPage}">
                                                <span
                                                    class="px-4 py-1.5 rounded-md bg-primary text-white text-xs font-bold shadow">${i}</span>
                                            </c:when>
                                            <c:when
                                                test="${i <= 2 || i >= totalPages - 1 || (i >= currentPage - 1 && i <= currentPage + 1)}">
                                            ...
                                            <c:when
                                                test="${(i == 3 && currentPage > 4) || (i == totalPages - 2 && currentPage < totalPages - 3)}">
